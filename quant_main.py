...
This script is the main entry point for the advanced quantitative trading system
that supports both IBKR TWS API (primary) and Alpaca API (fallback) with
automatic broker selection and manual switching capabilities.

Features:
- IBKR TWS API integration (professional-grade execution)
- Alpaca API fallback (reliable backup)
- Automatic broker selection with manual override
- Real-time and continuous trading modes
...
# Legacy Alpaca-only imports (for compatibility)
...
# Trading mode configuration for legacy system
...
def main():
    """Main entry point for the enhanced quantitative trading system."""
    
    # Set up command line arguments
    parser = argparse.ArgumentParser(description='Enhanced Quantitative Trading System (IBKR Primary + Alpaca Fallback)')
    parser.add_argument('--mode', type=str, choices=['historical', 'realtime', 'continuous'], default='historical',
                        help='Trading mode: historical (default), realtime (one-time), or continuous')
    ...
    parser.add_argument('--interactive', action='store_true',
                        help='Enable interactive broker switching during execution')
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("ENHANCED QUANTITATIVE TRADING SYSTEM")
        logger.info("=" * 60)
        logger.info(f"Mode: {args.mode}")
        logger.info(f"Strategy: {strategy_name}")
        logger.info(f"Symbols: {symbols}")
        logger.info(f"Broker preference: {args.broker}")
        
        # Choose trading system based on broker argument
        if args.broker == 'legacy':
            logger.info("🔄 Using LEGACY Alpaca-only system")
            # Use legacy Alpaca-only system
            trading_system = AlpacaTradingSystem()
            broker_type = "ALPACA (Legacy)"
        else:
            logger.info("🚀 Using ENHANCED unified system (IBKR + Alpaca)")
            # Use enhanced unified system
            force_broker = None if args.broker == 'auto' else args.broker.upper()
            trading_system = UnifiedTradingSystem(force_broker=force_broker)
            broker_type = trading_system.get_broker_type()
        
        # Display connection info
        logger.info(f"✓ Connected to: {broker_type}")
        
        if broker_type == "IBKR":
            logger.info("🎯 Using IBKR TWS API (PRIMARY)")
        elif "ALPACA" in broker_type:
            if args.broker == 'legacy':
                logger.info("⚙️  Using Legacy Alpaca system")
            else:
                logger.info("⚠️  Using Alpaca API (FALLBACK)")
        ...
        # Interactive broker switching if requested and using unified system
        if args.interactive and args.broker != 'legacy':
            logger.info("\n" + "=" * 40)
            logger.info("INTERACTIVE MODE")
            logger.info("=" * 40)
            if hasattr(trading_system, 'get_status'):
                status = trading_system.get_status()
                logger.info(f"Available brokers: {status['available_brokers']}")
                if len(status['available_brokers']) > 1:
                    logger.info("You can switch brokers during execution by pressing Ctrl+C and choosing 's'")
                else:
                    logger.info("Only one broker available - no switching possible")
        
        # Run the trading system based on mode
        logger.info("\n" + "=" * 40)
        logger.info(f"EXECUTING {args.mode.upper()} MODE")
        logger.info("=" * 40)
        ...
        if args.mode == 'continuous':
            ...
            trading_system.run_with_realtime_data(
                symbols, 
                strategy_name=strategy_name, 
                **strategy_params
            )
        ...
        logger.error("  1. IBKR TWS/Gateway is running and API is enabled")
        logger.error("  2. Alpaca API credentials are correct")
        ...
        try:
            if 'trading_system' in locals():
                if hasattr(trading_system, 'disconnect'):
